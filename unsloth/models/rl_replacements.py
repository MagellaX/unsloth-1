...
def grpo_trainer_compute_loss(function_name, function):
    if function_name != "compute_loss":
        return function
    # Newer TRL versions already route compute_loss through helper methods.
    # The legacy inline replacement below only applies to older implementations.
    if "_compute_loss(" in function or "_get_per_token_logps_and_entropies" in function:
        return function
